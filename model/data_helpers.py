...
def to_vec(tokens, vocab, maxlen):
    ...
    for i in range(n):
        ...
        if tokens[i] in vocab:
            ...
            vec.append(vocab["_unk_"])  # fix to fiance
    return length, np.array(vec)

def load_dataset(task_name, fname, vocab, max_utter_num, max_utter_len, max_response_len, max_persona_num, max_persona_len):

    # data loader for Persona-Chat
    def load_dataset_pc(fname, vocab, max_utter_num, max_utter_len, max_response_len, max_persona_num, max_persona_len):
        dataset=[]
        with open(fname, 'rt') as f:
            for us_id, line in enumerate(f):
                ...
                for utterance in utterances:
                    u_tokens = utterance.split(' ')[:max_utter_len]
                ...
                for response in responses:
                    r_tokens = response.split(' ')[:max_response_len]
                ...
                if fields[3] != "NA" and fields[4] == "NA":
                    personas = fields[3].split("|")[-max_persona_num:]
                    ps_tokens = []
                    ps_vec = []
                    ps_len = []
                    for persona in personas:
                        p_tokens = persona.split(' ')[:max_persona_len]
                ...
                # self persona
                if fields[3] == "NA" and fields[4] != "NA":
                    personas = fields[4].split("|")[-max_persona_num:]
                    ps_tokens = []
                    ps_vec = []
                    ps_len = []
                    for persona in personas:
    ...
    # data loader for CMU_DoG
    def load_dataset_cd(fname, vocab, max_utter_num, max_utter_len, max_response_len, max_persona_num, max_persona_len):
        dataset=[]
        with open(fname, 'rt') as f:
            for us_id, line in enumerate(f):
                line = line.strip()
    ...
    if task_name == "persona_chat":
        dataset = load_dataset_pc(fname, vocab, max_utter_num, max_utter_len, max_response_len, max_persona_num, max_persona_len)
    elif task_name == "cmu_dog":
        dataset = load_dataset_cd(fname, vocab, max_utter_num, max_utter_len, max_response_len, max_persona_num, max_persona_len)
...
def batch_iter(data, batch_size, num_epochs, max_utter_num, max_utter_len, max_response_num, max_response_len, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch_num in range(num_batches_per_epoch):
            ...
            yield np.array(x_utterances), np.array(x_utterances_len), np.array(x_utterances_num), np.array(x_utterances_char), np.array(x_utterances_char_len), \
                  np.array(x_responses), np.array(x_responses_len), np.array(x_responses_char), np.array(x_responses_char_len), \
                  np.array(x_personas), np.array(x_personas_len), np.array(x_personas_num), np.array(x_personas_char), np.array(x_personas_char_len), \
                  np.array(x_labels), x_ids
