...
from model.model_FIRE import FIRE


# Files
tf.flags.DEFINE_string("task_name", "", "task name")
...
tf.flags.DEFINE_float("gamma", 0.3, "threshold for knowledge filter")
tf.flags.DEFINE_integer("num_loop", 3, "number of loop in iteratively referring")

# Training parameters
tf.flags.DEFINE_integer("batch_size", 128, "batch size (default: 128)")
tf.flags.DEFINE_float("starter_learning_rate", 0.0005, "starter learning rate (default: 0.001)")
...
train_dataset = data_helpers.load_dataset(FLAGS.task_name, FLAGS.train_file, vocab, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_response_len, FLAGS.max_persona_num, FLAGS.max_persona_len)
print('train dataset size: {}'.format(len(train_dataset)))
valid_dataset = data_helpers.load_dataset(FLAGS.task_name, FLAGS.valid_file, vocab, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_response_len, FLAGS.max_persona_num, FLAGS.max_persona_len)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        grads_and_vars = optimizer.compute_gradients(fire.mean_loss)
        ...
        """
        loss_summary = tf.scalar_summary("loss", fire.mean_loss)
        acc_summary = tf.scalar_summary("accuracy", fire.accuracy)
        ...
        def train_step(x_utterances, x_utterances_len, x_utterances_num, x_u_char, x_u_char_len, 
            ...
            """
            feed_dict = {
              fire.utterances: x_utterances,
              fire.utterances_len: x_utterances_len,
              fire.utterances_num: x_utterances_num,
              fire.u_charVec: x_u_char,
              fire.u_charLen: x_u_char_len,
              fire.responses: x_response,
              fire.responses_len: x_response_len,
              fire.r_charVec: x_r_char,
              fire.r_charLen: x_r_char_len,
              fire.personas: x_personas,
              fire.personas_len: x_personas_len,
              fire.p_charVec: x_p_char,
              fire.p_charLen: x_p_char_len,
              fire.personas_num: x_personas_num,
              fire.target: x_target,
              fire.dropout_keep_prob: FLAGS.dropout_keep_prob
            }

            _, step, loss, accuracy, predicted_prob = sess.run(
                [train_op, global_step, fire.mean_loss, fire.accuracy, fire.probs],
                feed_dict)
        ...
        def dev_step():
            ...
            for valid_batch in valid_batches:
                ...
                feed_dict = {
                  ...
                  fire.personas_num: x_personas_num,
                  fire.target: x_target,
                  fire.dropout_keep_prob: 1.0
                }
                batch_accuracy, predicted_prob = sess.run([fire.accuracy, fire.probs], feed_dict)
        ...
        for batch in batches:
            x_utterances, x_utterances_len, x_utterances_num, x_u_char, x_u_char_len, \
              x_response, x_response_len, x_r_char, x_r_char_len, \
              x_personas, x_personas_len, x_personas_num, x_p_char, x_p_char_len, \
              x_target, x_ids = batch
            train_step(x_utterances, x_utterances_len, x_utterances_num, x_u_char, x_u_char_len, x_response, x_response_len, x_r_char, x_r_char_len, x_personas, x_personas_len, x_personas_num, x_p_char, x_p_char_len, x_target, x_ids)
